...
using MonoDevelop.Debugger.Win32;

namespace Mono.Debugger.Win32
{
    ...
    protected override void OnAttachToProcess(long procId)
    {
	  ...
	  void OnStdOutput (object sender, CorTargetOutputEventArgs e)
		...
		void OnBreak (object sender, CorThreadEventArgs e)
		{
			...
			args.Thread = GetThread ((CorThread) e.Thread);
		...
		void OnStepComplete (object sender, CorStepCompleteEventArgs e)
		{
			...
			if (ContinueOnStepIn (SequencePointExt.GetMethodInfo (e.Thread.ActiveFrame.Function, this))) {
				e.Continue = true;
				return;
			}
			...
			if (StepThrough (SequencePointExt.GetMethodInfo (e.Thread.ActiveFrame.Function, this))) {
			...
			if ((Options.StepOverPropertiesAndOperators || IsCompilerGenerated(SequencePointExt.GetMethodInfo (e.Thread.ActiveFrame.Function, this))) &&
			    IsPropertyOrOperatorMethod (SequencePointExt.GetMethodInfo (e.Thread.ActiveFrame.Function, this)) &&
		...
		{
			OnDebuggerOutput (false, string.Format ((string) "Unloaded Module '{0}'\n", (object) e.Assembly.Name));
			e.Continue = true;
		}
		...
		{
			OnDebuggerOutput (false, string.Format ((string) "Started Thread {0}\n", (object) e.Thread.Id));
			e.Continue = true;
		}

		void OnAssemblyLoad (object sender, CorAssemblyEventArgs e)
		{
			OnDebuggerOutput (false, string.Format ((string) "Loaded Module '{0}'\n", (object) e.Assembly.Name));
			e.Continue = true;
		}
		...
		private bool IsCatchpoint (CorException2EventArgs e)
		{
			...
			while (t != null) {
				exceptions.Add(SequencePointExt.GetTypeInfo(t, this).FullName);
				t = t.Base;
			}
					...
					if (bp != null) {
						if (bp is FunctionBreakpoint) {
							...
							if (doc.Reader is ISymbolReader2) {
								...
								if (methods != null && Enumerable.Any<ISymbolMethod> (methods)) {
									if (Enumerable.Count<ISymbolMethod> (methods) == 1) {
										met = methods [0];
									} else {
										int deepest = -1;
										foreach (var method in methods) {
											var firstSequence = SequencePointExt.GetSequencePoints (method).FirstOrDefault ((sp) => sp.StartLine != 0xfeefee);
		...
		public CorValue RuntimeInvoke (CorEvaluationContext ctx, CorFunction function, CorType[] typeArgs, CorValue thisObj, CorValue[] arguments)
		{
			...
			mc.OnInvoke = delegate {
				if (SequencePointExt.GetMethodInfo (function, this).Name == ".ctor")
			...
			mc.OnGetDescription = delegate {
				MethodInfo met = SequencePointExt.GetMethodInfo (function, ctx.Session);
				if (met != null)
...
/*				ValueReference<CorValue, CorType> msg = ctx.Adapter.GetMember (ctx, val, "Message");
		...
		public CorValue NewString (CorEvaluationContext ctx, string value)
		{
			...
			try {
				...
				if (doneEvent.WaitOne ((int) ctx.Options.EvaluationTimeout, false))
					return result;
				else
					return null;
			} finally {
		...
		string GetThreadName (CorThread thread)
		{
		    ...
		    { 
		        ...
		        if (val != null) 
		        { 
					Type classType = SequencePointExt.GetTypeInfo (val.ExactType, this);
		...
		protected override void OnSetNextStatement (long threadId, string fileName, int line, int column)
		{
			...
			MtaThread.Run (delegate {
				...
				ISymbolMethod met = SequencePointExt.GetSymbolMethod (frame.Function, this);
				if (met == null) {
					throw new NotSupportedException ();
				}
	...
	static class SequencePointExt
	{
		...
		public static Type GetTypeInfo (this CorType type, CorDebuggerSession session)
		{
			...
			if (type.Type == CorElementType.ELEMENT_TYPE_ARRAY || type.Type == CorElementType.ELEMENT_TYPE_SZARRAY) {
				...
				return MetadataExtensions.MakeArray (GetTypeInfo (type.FirstTypeParameter, session), sizes, loBounds);
			}

			if (type.Type == CorElementType.ELEMENT_TYPE_BYREF)
				return MetadataExtensions.MakeByRef (GetTypeInfo (type.FirstTypeParameter, session));

			if (type.Type == CorElementType.ELEMENT_TYPE_PTR)
				return MetadataExtensions.MakePointer (GetTypeInfo (type.FirstTypeParameter, session));
			...
			if (mi != null) {
				...
				if (targs.Length > 0) {
					...
					foreach (CorType ct in targs)
						types.Add (GetTypeInfo (ct, session));
